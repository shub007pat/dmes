...
function AddProcessStep() {
    ...
    const [processStep, setProcessStep] = useState({
        mainPartSerialNo: '',
        mainPartName: '',
        steps: [{ type: '', partSerialNo: '', partName: '', quantity: 0, operationDescription: '', units: '' }],
        addedBy: ''
    });
    ...
    const handleStepDetailChange = (index, field, value) => {
        // Check if the change is for the 'addedBy' field, which is not part of the steps array
        if (index === -1) {
            setProcessStep(prevState => ({
                ...prevState,
                [field]: value,
            ...
            // Otherwise, proceed to handle the change for a step
            const updatedSteps = [...processStep.steps];
    ...
    const handleSubmit = async (event) => {
        ...
        console.log('Submitting data:', processStep);  // Check the data structure
        try {
            const response = await axios.post('/api/process-steps', processStep);
            console.log('Server response:', response); // Log server response
            alert('Process step added successfully!');
            navigate('/process-steps');
        } catch (error) {
            console.error('Error adding process step:', error.response ? error.response.data : error);
            alert('Failed to add process step: ' + (error.response ? error.response.data.message : error.message));
        }
    };
