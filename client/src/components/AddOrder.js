...
function AddOrder() {
    const [orderData, setOrderData] = useState({
        orderBy: '',
        shipTo: '',
        parts: [{ partSerialNo: '', partName: '', quantity: 1, unitPrice: 0, totalAmount: 0 }],
        status: 'Pending'
    });
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...orderData.parts];
        list[index][name] = name === 'quantity' ? parseInt(value) : value;
        if (name === 'quantity') {
            list[index].totalAmount = list[index].unitPrice * value;
        }
        setOrderData({
            ...orderData,
            parts: list
    ...
    const handleAddClick = () => {
        setOrderData({
            ...orderData,
            parts: [...orderData.parts, { partSerialNo: '', partName: '', quantity: 1, unitPrice: 0 }]
        });
    };
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            alert('Failed to add order: ' + error.response.data.message);
        }
    };
